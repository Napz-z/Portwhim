...
export function HealthCheck({row}:{row:Listener}){
  ...
  return <section className="inspector-section health-check"><h3>Connection check</h3><p>Manually check TCP connectivity and an HTTP HEAD response from localhost.</p>
    ...
    <AnimatePresence initial={false}>{result&&<motion.div initial={{opacity:0,height:0,y:reduced?0:8}} animate={{opacity:1,height:"auto",y:0}} exit={{opacity:0,height:0}} transition={reduced?{duration:0}:spring} className="health-result" role="status"><strong>{result.status!==null?`HTTP ${result.status}`:result.connected?'TCP connected · HTTP unavailable':'Connection failed'}</strong><small>{result.elapsedMs} ms · {new Date(result.checkedAt).toLocaleTimeString()}</small><code>{result.url}</code><p>{result.message}</p></motion.div>}</AnimatePresence>
