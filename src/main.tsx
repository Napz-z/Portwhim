  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <div className="content">
        ...
        {(watchError||watchStatus.error||watchStatus.notificationError)&&<p className="notice" role="alert">{watchError||watchStatus.error||watchStatus.notificationError}</p>}
        {favorites.some(f=>f.watch)&&<p className="watch-status">{watchStatus.scannedAt?'Port watch checked '+new Date(watchStatus.scannedAt).toLocaleTimeString():'Port watch is establishing its first successful baseline…'} · Independent of explorer pause. Desktop delivery follows system notification settings.</p>}
        {watchStatus.alerts.length>0&&<details className="watch-alerts"><summary>Port watch · {watchStatus.alerts.length} recent alerts</summary><ul>{watchStatus.alerts.map((alert,index)=><li key={alert.at+':'+index}><time>{new Date(alert.at).toLocaleTimeString()}</time><button onClick={()=>{setQuery(':'+alert.port);setProtocol(alert.protocol);setFilter('all');}}>{alert.message}</button></li>)}</ul></details>}
        {parsedQuery.error&&<p className="query-note" role="alert">{parsedQuery.error}</p>}
        {isTauri()&&<div className="search-summary"><button onClick={()=>void invoke('hide_to_tray').catch(e=>setToast(String(e)))}>Hide to tray</button>Explorer refreshes every 30s in the background; watched ports every 5s. Closing the window quits the app.</div>}
        ...
        <section className="explorer" data-view={view}>
    ...
    <MotionPresence show={help}>{help&&<div className="modal-shade" onClick={()=>setHelp(false)}><section className="help" role="dialog" aria-modal="true" aria-label="About Portwhim" onClick={e=>e.stopPropagation()}><button className="icon-button close" aria-label="Close about" onClick={()=>setHelp(false)}><X size={18}/></button><Network size={32}/><h2>Small tool. Clearer localhost.</h2><p>Portwhim {version} reads local socket and process metadata. No account, telemetry, or remote scanning.</p><p><kbd>Ctrl / ⌘ K</kbd> Search · <kbd>Esc</kbd> Close inspector</p><p>UDP entries are bound sockets, not TCP-style listeners. Port hints are suggestions, not verified service identities. Network-bound sockets may not serve HTTP on localhost.</p><p>CPU and memory are per process, shared by all its sockets. Missing OS metadata appears as —. Stop affects the whole process and asks for confirmation.</p></section></div>}</MotionPresence>
    <MotionPresence show={!!toast}>{toast&&<div className="toast" role="status"><Check size={16}/>{toast}</div>}</MotionPresence>
  </div>;
}
