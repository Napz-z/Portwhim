...
function App(){
  ...
  const rows=sortListeners(all.filter(l=>matchesOwnership(l,filter)&&(protocol==='all'||l.protocol===protocol)&&`${l.port} ${l.pid} ${l.name} ${l.service} ${l.address} ${l.provenance.project?.name||""} ${l.provenance.project?.directory||""}`.toLowerCase().includes(query.toLowerCase())),sort);
