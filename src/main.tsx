...
function App(){
  ...
  const rows=all.filter(l=>(filter==='all'||filter==='dev'&&l.category!=='system'||filter==='network'&&l.scope!=='loopback')&&(protocol==='all'||l.protocol===protocol)&&`${l.port} ${l.pid} ${l.name} ${l.service} ${l.address}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>sort==='port'?a.port-b.port:(b.memory??-1)-(a.memory??-1));
  ...
  return <div className="app-shell">
    ...
    {row&&<><div className="drawer-shade" onClick={()=>setSelected('')}/><aside className="drawer" aria-label="Process details"><div className="drawer-top"><span>PROCESS INSPECTOR</span><button className="icon-button" aria-label="Close details" onClick={()=>setSelected('')}><X size={19}/></button></div><span className={`service-icon large ${row.category}`}>{icon(row)}</span><h2>{row.service}</h2><p className="detail-sub">{row.name} <span className="protocol">{row.protocol}</span></p><div className="detail-port">:{row.port}<button className="icon-button" title="Copy port" onClick={()=>void action('copy',row)}><Copy size={17}/></button></div><div className="detail-actions"><button className="primary" disabled={acting||row.protocol!=='TCP'} onClick={()=>void action('open',row)}>Open localhost <ArrowUpRight size={16}/></button><button className="secondary" onClick={()=>void action('copy',row,'pid')}><Copy size={15}/> PID</button></div><dl>{[['Process ID',row.pid],['Bind address',row.address],['Scope',row.scope],['Started',time(row.started)],['CPU',cpu(row.cpu)],['Memory (RSS)',memory(row.memory)],['Recognition',row.confidence==='process'?'Process / command signature':row.confidence==='port hint'?'Port hint · not verified':'No known signature']].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl><div className="related"><h3>Other sockets in this process</h3><div>{all.filter(l=>l.pid===row.pid&&identity(l)!==identity(row)).map(l=><button key={identity(l)} onClick={()=>setSelected(identity(l))}>:{l.port} <small>{l.protocol}</small></button>)}{all.filter(l=>l.pid===row.pid).length===1&&<p>No other sockets.</p>}</div></div><div className="stop-area"><p>Stopping a process closes all of its ports.</p><button className="danger" disabled={!row.canStop||acting} onClick={()=>void action('stop',row)}><Square size={14}/> Stop process</button>{!row.canStop&&<small>Protected process or identity unavailable.</small>}</div></aside></>}
