...
function App(){
  ...
  const rows=all.filter(l=>matchesOwnership(l,filter)&&(protocol==='all'||l.protocol===protocol)&&`${l.port} ${l.pid} ${l.name} ${l.service} ${l.address} ${l.provenance.project?.name||""} ${l.provenance.project?.directory||""}`.toLowerCase().includes(query.toLowerCase())).sort((a,b)=>sort==='port'?a.port-b.port:(b.memory??-1)-(a.memory??-1));
  ...
  async function action(type:'copy'|'open'|'stop',l:Listener,field:'pid'|'port'='port'){
    ...
    try{
      ...
      if(type==='stop'){if(!canRequestStop(l))throw new Error(l.stopReason||'Stopping is disabled for this process.');const result=await window.portwhim.stop(l.id);if(!result.cancelled){setToast(result.message||'Stop requested');await refresh();}}
