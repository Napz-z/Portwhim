...
搜索还支持 `:3000`、`port:3000`、`http://localhost:3000` 等本地 URL，以及 `pid:1234`。按 Enter 打开首条结果；如果协议或归属筛选隐藏了匹配项，搜索摘要可以恢复全部匹配。TCP/UDP 命名收藏保存在本机（最多 32 个），没有当前监听的端口也可以收藏。

应用在当前会话保留最近 20 条端口变化和停止结果。前台每 5 秒刷新，后台每 30 秒刷新。显式使用 **Hide to tray** 可隐藏窗口；托盘展示收藏端口状态并支持快速定位，关闭窗口仍会退出应用。

**Docker published ports** 是手动、只读的本地 Docker 引擎检查，显示容器名、主机/容器端口映射和 Compose 项目，不提供停止容器功能。需要 Docker CLI 和正在运行的本地引擎；远程 context、自定义/rootless 套接字及网络可达性不在范围内。
...
## 开发中的功能（尚未包含在 v0.1.2 安装包中）

- **项目总览：** 点击列表 / 进程视图旁的文件夹按钮，按 Git 工作目录归组，显示可读取的分支与 worktree；没有 Git 信息时使用识别到的项目目录。可在本机保存最多 64 个命名的预期 TCP/UDP 服务，区分没有监听与端口被其他归属占用。无法归属的进程分别显示。
- **收藏监看：** 为收藏逐个开启 **Watch**。应用运行期间，原生后台任务每 5 秒检查，包括隐藏到托盘和暂停列表刷新时。首次成功扫描只建立基准；变化须持续至少 10 秒，同一端口的提醒间隔至少 60 秒。同项目同进程名的 PID 变化保持安静，以减少热重载干扰；缺少项目归属时按进程身份判断。扫描失败不会当成服务消失。当前会话保留最近 20 条提醒。通知受系统设置影响，Windows 需要安装后的应用才能验证通知送达。
- **重启诊断：** 详情结合可见父进程链、管理工具特征与最近一次停止目标，提示去 nodemon、Node watch、PM2、系统服务管理器或 Docker 排查。提示是线索，不证明重启原因，也不会自动反复结束进程。
- **连接检查：** 在详情手动选择 HTTP / HTTPS，测试本机 TCP 连接，再向 `/` 发送 HEAD 请求，展示状态码、耗时与检查时间。仅检查可经回环访问的 TCP 绑定；不跟随重定向、不使用代理、不读取响应正文，保留证书验证。拒绝 HEAD、身份验证错误和 HTTP 5xx 都如实显示，不把响应等同于健康。TCP 和 HTTP 超时分别为 2 秒与 4 秒。
- **交互：**弹簧式选中指示、列表位置过渡、视图切换及面板进出动画。收藏与活动记录收进精简工具栏，表单按需展开。面板支持键盘焦点限制与恢复；About & shortcuts 可选择跟随系统、完整动画或减少动画。

需要重新构建源码才能使用；实际完成的检查见[验证记录](VALIDATION.md)。
...
- CI 构建成功不代表各平台的安装与全部功能都已验证。实际测试范围见[验证记录](VALIDATION.md)。
...
点击端口或 PID 即可复制，按 `Esc` 关闭详情。文字查询为模糊匹配，数字和 `port:` 查询始终精确匹配端口，不用于查询 PID；也可用 `pid:` 精确匹配进程。搜索时已有筛选条件继续生效。
