...
TCP 监听与 UDP 绑定（IPv4/IPv6）、进程与资源详情、服务识别、搜索和组合筛选、端口/内存排序、进程卡片、关联端口、复制端口/PID、打开 localhost、确认后停止进程、自动/手动刷新与暂停。
...
停止会结束整个进程，影响其全部端口；Windows 立即终止，Unix 发送 SIGTERM，不自动强杀。不允许结束自身、其后代、父进程、受保护或无法核实身份的进程。操作前后核对身份，原生对话框确认。父进程信息来自当前快照，不是历史启动记录。
