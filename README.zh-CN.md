...
输入 `3000`、`:3000` 或 `port:3000` 精确匹配端口（1–65535），不会匹配 PID 或路径；文字仍支持模糊搜索。协议与分类筛选继续生效，空结果仅表示当前扫描和筛选下未发现绑定，不保证端口可用。

已识别项目的详情提供 Open folder 与 Copy path。打开前检查目录存在；目录未知时不显示操作，目录已删除或无权访问时提示错误。复制路径使用已识别的项目路径。

首次成功扫描建立基线，之后成功扫描对比 TCP/UDP 端口及进程身份，显示最新一批新增、消失或占用进程变化。摘要最多显示三项，可关闭，不保存历史、不发送系统通知。双栈地址去重，资源变化不会触发提示；暂停期间不扫描，恢复后对比最后成功快照。扫描间隙的短暂变化可能无法捕获。
