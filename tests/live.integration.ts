...
test('real TCP / UDP discovery, identity check, termination of owned fixture only', {timeout:60000}, async()=>{
  const fixture=spawn(process.execPath,['-e',`const net=require('node:net'),udp=require('node:dgram').createSocket('udp4');const s=net.createServer();s.listen(0,'127.0.0.1',()=>udp.bind(0,'127.0.0.1',()=>console.log(JSON.stringify({tcp:s.address().port,udp:udp.address().port}))));`],{stdio:['ignore','pipe','pipe']});
  try{
  ...
  }finally{if(fixture.exitCode===null&&!fixture.killed)fixture.kill();}
});
